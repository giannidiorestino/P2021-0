...
\begin{exercise} Prove the Fatou theorem for sets and for non-negative
    functions.
  \end{exercise}
  ...
  \begin{exercise}
    ...
    \end{equation*}
    The $j$-th trial is the mapping $X_j : \Omega \to S=\set{0,1}$,
    $X_j(\omega) = x_j$ if $x_j$ is the $j$-th element pf $\omega$. Let
    $\mathcal F$ be the $\sigma$-algebra generated by $X_j$, $j \in
    \naturals$ and $\mathcal F_n$ the $\sigma$-algebra generated by
    $X_1,\dots,X_n$, $n=1,2,\dots$. Consider the random varibles
    \begin{equation*}
      F_t = \frac 1 t \sum_{j=1}^t X_j \ , \quad F_t \colon \Omega \to
      \setof{\frac k n}{\sum_{j=1}^n X_j = k} \ .
    \end{equation*}
Show that the set
\begin{equation*}
  \setof{\omega}{\lim_{n\to\infty} F_n = \frac 12}
\end{equation*}
is in $\mathcal F$.
...
$S$, that is $S \circ S^+(\theta) = \theta$, $\theta \in [0,1[$; 3)
such that the push-forward of the Lebesgue measure is a measure $\mu$
